---
title: "Binomial approximation to 538 and the Economist's predictions"
author: "Matthew Kay"
output: github_document
---

To build a Plinko board / Galton board for the Economist and 538 predictions, we need binomial distributions that can approximate
the current predictions. In this document we'll find such distributions.

Basically, what we want is a bin width, number of bins, and
a mean that we then throw into the Plinko board simulator from [plinko](https://mjskay.github.io/plinko/).
...
automatic slot height calculation in `plinko::plinko_board()` so that both boards
have slots of the same height. To do that, we need to take the height of the
highest bin across both boards and add a bit of leeway:
...
Looks good! And the final distribution looks plausible given the theoretical
Binomial distribution. Let's build the animation:

```{r}
board_538 %>%
  animate(width = width_px, height = height_px, fps = 30, end_pause = 6 * 30, cores = 7)
```
...
We also want animations for each ball, so the website can have a "drop a random
ball" button:
...
make_single_ball_boards = function(model_name, ev_quantiles, ...) {
  for (i in 1:n_ball) {
    ...
    animation = make_plinko_board(model_name, ev_quantiles[[i]], ...) %>%
      animate(width = width_px, height = height_px, fps = 30, renderer = gganimate::gifski_renderer(loop = FALSE), cores = 7)
...
board_econ %>%
  animate(width = width_px, height = height_px, fps = 30, end_pause = 6 * 30 + extra_frames, cores = 7)
```
...
agg_png("galton_board-explainer.png", width = width_px * 2, height = height_px, res = 100)
last_plot()
dev.off()
```
