...
bin_width = 39
board_mean = 328.5
model_name = "the Economist's model"
...
starting_x = qunif(ppoints(n_draw, a = 0.75), -bin_width/2, bin_width/2) + board_mean
starting_x = sample(starting_x, length(starting_x))

# figure out aspect ratio
height_px = 700
width_px = (max(slot_edges, x_max) - min(slot_edges, x_min))/(board_height + 10 * bin_width) * height_px
...
for (i in 1:n_frame) {
  ...
  p = ggplot(circles) + 
    ...
    theme(
      ...
      axis.title.x = element_text(hjust = 0, size = 10, color = "gray50")
    ) +
