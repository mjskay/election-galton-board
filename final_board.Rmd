...
```{r}
uncalled_states = setdiff(names(ev), c(biden_states, trump_states))
uncalled_states
```
...
format_state_list = function(states) {
  if (is.null(states)) {
    ...
    paste(stringi::stri_wrap(paste(sort(states), collapse = " "), width = 33, indent = 7), collapse = "<br>")
  }
}

final_ev = sum(ev[biden_states])
  ...
  plinko_board(..., 
    ...
    add_layers(
      ...
      annotate("richtext",
        x = 650, y = 0,
        label = paste0(
          "<strong style='color: black;'>Presidential-Plinko.com Results Board</strong><br><br>",
          "The height of the board shows the uncertainty<br>",
          "in the number of electoral votes (EVs) predicted for<br>",
          "Biden <strong>at the beginning of election night</strong><br>",
          "(per the Economist). The ball is dropped from<br>",
          "the average number of EVs he was predicted<br>",
          "to get, landing at <strong style='color: ", Biden_color, "'>the current result:</strong><br><br>",
          "Biden (", sum(ev[biden_states]), "):", format_state_list(biden_states), "<br><br>",
          "Trump (", sum(ev[trump_states]), "):", format_state_list(trump_states), "<br><br>",
          "Uncalled (", sum(ev[uncalled_states]), "):", format_state_list(uncalled_states), ""
        ),
        color = "gray25",
      ...
      annotate("polygon",
        x = mean_ev_adj_econ + c(-10, +10, +10, +20,   0, -20, -10),
        y = total_height *     c(.90, .90, .87, .87, .85, .87, .87),
        fill = "black",
        alpha = 0.2
      ),
      stat_histinterval(
...
```{r}
board_econ %>%
  animate(width = width_px, height = height_px, fps = 15, cores = 7, end_pause = 6 * 15)
```
